...
import { ClientToServerEvents, ServerToClientEvents, SocketData, RoomJoinPayload, PlaybackActionPayload } from '../types/socket.types';
...
export const registerRoomHandlers = (
  ...
  socket.on('room:join', async (payload: RoomJoinPayload) => {
    try {
      ...
      socket.emit('app:error', { message: error.message || 'Failed to join room' });
    }
  });
  ...
  socket.on('room:leave', async (payload: RoomJoinPayload) => {
    try {
      ...
      socket.emit('app:error', { message: error.message || 'Failed to leave room' });
    }
  });
  ...
  socket.on('playback:action', async (payload: PlaybackActionPayload) => {
    try {
      ...
      logger.error(`Playback action failed for ${user.username}: ${error.message}`);
      socket.emit('app:error', { message: error.message || 'Playback action failed' });
    }
  });
